...
<ul id="<?= esc_attr($id); ?>" class="<?= esc_attr($className); ?>">
    <?php if ($fileQuery->have_posts()): ?>
        <?php while ($fileQuery->have_posts()): $fileQuery->the_post(); ?>
            <li>
                <a class="file" href="<?= esc_url(get_field('file', get_the_ID())); ?>" target="_blank">
                <!-- <a class="file" href="<?php //the_permalink(); ?>" target="_blank"> --> <!-- to use with S3 to mask the permalink -->
