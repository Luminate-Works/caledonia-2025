...
// ===============================================
// Block Template: Press Coverage Filter by Category + post_sector
// with ACF toggles
// ===============================================

// Set block ID and classes.
$id = $block['anchor'] ?? 'lmn-' . $block['id'];
$className = 'press-coverage-filter'
...
// Get years from press-coverage
global $wpdb;
$years = $wpdb->get_col("
	SELECT DISTINCT YEAR(post_date) 
	FROM {$wpdb->posts} 
	WHERE post_type = 'press-coverage' 
...
if (is_admin()) {
    echo '<p><strong>Press Coverage Filter Block</strong></p>';
    return;
}
?>
...
<div
    ...
    <?php if ($show_filter): ?>
        <div class="press-coverage-filter-controls">

            <div class="inner">
                <!-- <span class="press-coverage-filter-label">
                    Filter by
                </span> -->
                ...
                <?php if ($show_search): ?>
                    <div class="search-bar">
                        <input type="text" x-model="searchQuery" placeholder="Search press coverage" autocomplete="off" />
                    </div>
                <?php endif; ?>
    ...
    <div class="press-coverage-listing" :class="{ 'limited': limitNumber }">
        <template x-for="item in pressCoverageWithSeparators()" :key="item.id">
            <div
...
<script>
    function pressCoverageApp(data = {}) {
        return {
            posts: [],
            offset: 0,
            postsPerPage: 6,
            ...
            loadPressCoverage(reset = false, customLimit = null) {
                if (this.loading) return;
                this.loading = true;
                const fd = new FormData();
                fd.append('action', 'load_wp_press_coverage');
